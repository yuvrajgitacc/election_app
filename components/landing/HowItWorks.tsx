...
export default function HowItWorks() {
  ...
  return (
    <section style={{ width: '100%', background: '#f8f9fa', padding: '120px 24px' }}>
          ...
          {cards.map((card, index) => {
            ...
            return (
              <motion.div
                ...
                style={{ background: '#ffffff', border: '1px solid var(--border)', borderRadius: '20px', padding: '32px', position: 'relative', overflow: 'hidden', cursor: 'pointer', transition: 'all 0.3s ease' }}
              >
                <div className="absolute top-[-8px] right-[16px] z-0 pointer-events-none select-none">
                  <span className="font-display text-[96px] text-[#f1f3f5]">
                    {card.number}
                  </span>
                </div>

                <div className="relative z-10 w-[48px] h-[48px] bg-[#f8f9fa] rounded-[12px] flex items-center justify-center mb-[24px]">
                  <Icon className="w-[24px] h-[24px] text-[#5b6ef5]" />
                </div>

                <h3 className="relative z-10 text-[20px] text-[#111118] font-medium mb-[8px]">
                  {card.title}
                </h3>
                <p className="relative z-10 text-[14px] text-[#4b5563] leading-[1.7]">
                  {card.description}
                </p>
              </motion.div>
            );
